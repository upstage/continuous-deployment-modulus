...
var rmdir = function(path, cb) {
  runCmd('rm -rf ' + path + '/', function(err) {
    if(err) {
      console.log('FAILED...');
      console.log(err);
      cb(err);
...
var getFormattedDateTime(datetime) {
...
var middleware = function(app) {
  return function (req, res, next) {
    ...
    var loginCmd = './node_modulus/modulus/bin/modulus login' 
                  + (useGithub === true ? ' --github' : '')
                  + ' --username ' + username
                  + ' --password ' + password;

    var deployCmd = './node_modulus/modulus/bin/modulus deploy ./' + tmpPath
                  + ' -p ' + modulusProject;
